...
public class TaskDAO {
    ...
    public List<Task> getAll(int offset, int limit) { // на сколько сместиться от начала резул, сколько за раз показать резулт
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
    public int getAllCount() { // сколько всего есть задач для pages
        Query<Long> query = getSession().createQuery("select count(t) from Task t", Long.class);
        // превышение int, ошибка
